...
thiolate molecules in the following simulations, and missing parameters for the
...
surfaces, the S parameters from Luedtke and
Landman\cite{landman:1998} were adopted and modified the parameters for the CTS
...
Four sizes of nanoparticles ($R =$ 10, 15, 20, and 25
\AA) were modelled. The smallest particle size produces the lowest interfacial
...
Stocker \textit{et al.} observed that ligand-solvent alignment was an important factor for
heat transfer into the solvent.\cite{Stocker:2013cl}
With high surface curvature and lower
...
Similarly, the distribution of \emph{hexane} molecule
orientations relative to the particle surface using the same angular
analysis utilized for the ligand chain orientations was examined. In this case,
...
particle and the end of the ligand chains are selected. A large population of
...
To estimate the relevant densities of states, the
velocity of each atom $i$ in the region of the interface is projected onto a
